/**
 * Diccionario DIVIPOLA - División Político-Administrativa de Colombia
 * Códigos de departamentos y municipios según DANE
 */
...
// Municipios de Colombia (código DANE -> nombre)
// Formato: código de 5 dígitos donde los 2 primeros son el departamento
...
// Fuentes de financiación conocidas
export const FUENTES_FINANCIACION: Record<string, string> = {
  'PGN': 'Presupuesto General de la Nación',
  'COLSMART': 'Colombia Smart',
  'SGR': 'Sistema General de Regalías',
  'BID': 'Banco Interamericano de Desarrollo',
  'BM': 'Banco Mundial',
  'CATMUL': 'Catastro Multipropósito',
  'CONV': 'Convenio',
  'COOP': 'Cooperación Internacional',
};

// Etapas operativas conocidas
export const ETAPAS_OPERATIVAS: Record<string, string> = {
  '01_pre': 'Preoperativa',
  '02_oper': 'Operativa',
  '03_post': 'Postoperativa',
  '04_bk': 'Backup',
};
 ...
 */
export function getNombreDepartamento(codigo: string): string {
  // Normalizar código (puede venir como "20" o "20")
  const codigoNormalizado = codigo.padStart(2, '0');
  return DEPARTAMENTOS[codigoNormalizado] || codigo;
}

/**
 * Obtiene el nombre del municipio por código
 */
export function getNombreMunicipio(codigo: string): string {
  // Normalizar código (puede venir como "295" o "20295")
  let codigoNormalizado = codigo;

  // Si es código corto (3 dígitos), necesitamos el código del departamento
  if (codigo.length <= 3) {
    return MUNICIPIOS[codigo] || codigo;
  }

  // Código completo de 5 dígitos
  codigoNormalizado = codigo.padStart(5, '0');
  return MUNICIPIOS[codigoNormalizado] || codigo;
}

/**
 * Obtiene el nombre del municipio dado código de departamento y municipio
 */
export function getNombreMunicipioCompleto(codigoDpto: string, codigoMun: string): string {
  const codigoCompleto = codigoDpto.padStart(2, '0') + codigoMun.padStart(3, '0');
  return MUNICIPIOS[codigoCompleto] || codigoMun;
}

/**
 * Obtiene el nombre de la fuente de financiación
 */
export function getNombreFuente(codigo: string): string {
  return FUENTES_FINANCIACION[codigo.toUpperCase()] || codigo;
}

/**
 * Obtiene el nombre de la etapa operativa
 */
export function getNombreEtapa(codigo: string): string {
  return ETAPAS_OPERATIVAS[codigo.toLowerCase()] || codigo;
}

/**
 * Obtiene todos los municipios de un departamento
 */
export function getMunicipiosPorDepartamento(codigoDpto: string): Array<{codigo: string; nombre: string}> {
  const prefijo = codigoDpto.padStart(2, '0');
  const municipios: Array<{codigo: string; nombre: string}> = [];

  for (const [codigo, nombre] of Object.entries(MUNICIPIOS)) {
    if (codigo.startsWith(prefijo)) {
      municipios.push({
        codigo: codigo.substring(2), // Solo los 3 últimos dígitos
        nombre
      });
    }
  }
...
/**
 * Información parseada de una ruta de acceso
 */
export interface ParsedAccessPath {
  codigoDepartamento?: string;
...
/**
 * Parsea una ruta de acceso y extrae los códigos de departamento, municipio, fuente y etapa
 *
 * Ruta esperada (Linux): 01_actualiz_catas/{depto}/{mun}/{fuente}/{etapa}
 * Ruta esperada (Windows): 01_actualiz_catas\{depto}\{mun}\{fuente}\{etapa}
 *
 * Ejemplo: 01_actualiz_catas/20/295/PGN/03_post
 *   - Departamento: 20 (Cesar)
 *   - Municipio: 295 (Gamarra)
 *   - Fuente: PGN
 *   - Etapa: 03_post
 */
export function parseAccessPath(path: string): ParsedAccessPath {
  // Normalizar separadores (Windows -> Linux)
  const normalizedPath = path.replace(/\\/g, '/').replace(/^\/+/, '');
  const parts = normalizedPath.split('/').filter(p => p);

  // Buscar el índice de 01_actualiz_catas
  const catastroIndex = parts.findIndex(p => p === '01_actualiz_catas');

  if (catastroIndex === -1 || parts.length <= catastroIndex + 1) {
    return { isFilterable: false };
  }

  const result: ParsedAccessPath = { isFilterable: true };

  // Departamento (posición después de 01_actualiz_catas)
  if (parts.length > catastroIndex + 1) {
    result.codigoDepartamento = parts[catastroIndex + 1];
    result.nombreDepartamento = getNombreDepartamento(result.codigoDepartamento);
  }

  // Municipio (siguiente posición)
  if (parts.length > catastroIndex + 2) {
    result.codigoMunicipio = parts[catastroIndex + 2];
    if (result.codigoDepartamento) {
      result.nombreMunicipio = getNombreMunicipioCompleto(result.codigoDepartamento, result.codigoMunicipio);
    }
  }

  // Fuente de financiación (siguiente posición)
  if (parts.length > catastroIndex + 3) {
    result.fuente = parts[catastroIndex + 3];
    result.nombreFuente = getNombreFuente(result.fuente);
  }

  // Etapa operativa (siguiente posición)
  if (parts.length > catastroIndex + 4) {
    result.etapa = parts[catastroIndex + 4];
    result.nombreEtapa = getNombreEtapa(result.etapa);
  }
